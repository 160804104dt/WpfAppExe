...
使用：ItemTemplateSelector = "{StaticResource 模板选择器名称}"
...
#### 5.listbox.ItemContainerGenerator.ContainerFromItem(item)

得到指定条目所在的控件，item为listbox数据源中的某一项
...
普通的属性是直接从类中的私有成员读取的，而依赖属性是从DependencyObject类中的GetValue方法时动态解析的。

设置依赖属性的值的时候，它不会存储到字段中，而是存在DependencyObject提供的键值对的字典中，键是属性的名称，值是设置的值。

当访问依赖属性时，会调用GetValue方法：如果字典中的有值，就从字典中读取，如果没有，就向上查找逻辑树，并找到继承的值，如果还没有，就采用属性元数据中定义的默认值
...
InvokeCommandAction　各种事件，例如：鼠标，键盘，焦点等
...
WPF 应用程序从两个线程开始：一个用于处理呈现，另一个用于管理 UI。 当 UI 线程接收输入、处理事件、绘制屏幕和运行应用程序代码时，呈现线程通过隐藏方式在后台高效运行。 大多数应用程序使用单个 UI 线程，不过在某些情况下，最好使用多个线程。 我们将稍后通过示例对此进行讨论。
